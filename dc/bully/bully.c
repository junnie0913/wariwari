...
int is_new_coord = 1; // assume you are the winner until you
lose
...
is_new_coord = 0; // a proc with id > self
exists thus cannot be coord
}
}
return is_new_coord;
}
...
sendto(sock_id,(const char *)message,strlen(message),MSG_CONFIRM,(const struct sockaddr *)&from,sizeof(from));
...
else if (!strcmp(buff, "E-ACK")) continue; // nothing do, your job is done
else if (!strcmp(buff, "COORDINATOR")) bully_id = from.sin_port;
}
}